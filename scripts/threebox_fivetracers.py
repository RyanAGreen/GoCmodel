...
class ODEBoxModel:  # three box ocean model with circulation, bio pump, and Sr tracers
    def __init__(self):
        ...
        self.ocean_mass = (
            9.48024 * 10 ** 17
        )  # kg -> calculated from GoC volume (1.45e+14 m3) + Baja volume (1.45e+14*5 m3) * density of sw (kg/m3)
        self.m0 = (
        ...
        self.stateV0 = np.hstack(
            (self.DIC, self.ALK, self.P, self.N, self.d13C, self.D14C)
        )
        self.BC = np.array(
            [
                [3000e-6 * self.m0, 3000e-6 * self.m0],
                [3400e-6 * self.m0, 3400e-6 * self.m0],
                [3e-6 * self.m0, 3e-6 * self.m0],
                [35e-6 * self.m0, 35e-6 * self.m0],
                [-0.1, -0.1],
                [200, 200],
            ]
        )  # (tracers,boxes) for boundary condition

        self.SvM = self.circ(0.45, 0.005)  # Sv 0.45,0.05
    ...
    def makeTM(self, SvM):
        ...
        dt = 1  # timestep (yr)
        flux = (
            SvM * (1e6 * 1026 * 3.154e7) * dt
        )  # conversion from Sv (10e6 m3/s) to kg/yr moved in 1 timestep
        m_lost = np.sum(flux, axis=0)  # sum of all mass fluxes out of each box
        fraction_retained = (
            self.m - m_lost
        ) / self.m  # fraction of mass retained in each box

        # wouldnt this be kg / kg ??
        fractional_fluxes = flux / self.m.reshape(
            (len(self.m), 1)
        )  # divide flux array rows by mass for concentration
        fractional_fluxes_inv = (
            flux / self.m.T
        )  # divide flux array columns by mass for inventory
        TM_ForConcentrations = fractional_fluxes + np.diag(
            fraction_retained
        )  # TM_ForConcentrations is NxN matrix defining the fractional mixing system of equations for concentration units, representing 1 year of ocean circulation
        TM_ForInventories = fractional_fluxes_inv + np.diag(
            fraction_retained
        )  # TM_ForInventories is NxN matrix defining the fractional mixing system of equations for inventory units, representing 1 year of ocean circulation
        return TM_ForConcentrations - np.identity(
            self.Nbox + self.Nbc
        )  # , TM_ForInventories
    ...
    def MakePlot(self):
        ...
        ax[1].plot(
            self.time, self.result.y[0, :] / self.m[0], label="Baja California C"
        )
        ax[2].plot(
            self.time, self.result.y[1, :] / self.m[0], label="Baja California ALK"
        )
        ax[0].plot(
            self.time, self.result.y[3, :] / self.m[0], label="Baja California N"
        )
        ax[3].plot(
            self.time,
            self.result.y[4, :] / self.m[0],
            label="Baja California δ$^{13}$C",
        )
        ax[4].plot(
            self.time,
            self.result.y[5, :] / self.m[0],
            label="Baja California ∆$^{14}$C",
        )

        ax[1].plot(
            self.time,
            self.result.y[6, :] / self.m[1],
            linestyle="dotted",
            label="GoC deep C",
        )
        ax[2].plot(
            self.time,
            self.result.y[7, :] / self.m[1],
            linestyle="dotted",
            label="GoC deep ALK",
        )
        ax[0].plot(
            self.time,
            self.result.y[9, :] / self.m[1],
            linestyle="dotted",
            label="GoC deep N",
        )
        ax[3].plot(
            self.time,
            self.result.y[10, :] / self.m[1],
            linestyle="dotted",
            label="GoC deep δ$^{13}$C",
        )
        ax[4].plot(
            self.time,
            self.result.y[11, :] / self.m[1],
            linestyle="dotted",
            label="GoC deep ∆$^{14}$C",
        )
        ...
        ax[1].plot(
            self.time,
            self.result.y[12, :] / self.m[2],
            linestyle="dashed",
            label="GoC surface C",
        )
        ax[2].plot(
            self.time,
            self.result.y[13, :] / self.m[2],
            linestyle="dashed",
            label="GoC surface ALK",
        )
        ax[0].plot(
            self.time,
            self.result.y[15, :] / self.m[2],
            linestyle="dashed",
            label="GoC surface N",
        )
        ax[3].plot(
            self.time,
            self.result.y[16, :] / self.m[2],
            linestyle="dashed",
            label="GoC surface δ$^{13}$C",
        )
        ax[4].plot(
            self.time,
            self.result.y[17, :] / self.m[2],
            linestyle="dashed",
