...
class ODEBoxModel(): # three box ocean model with circulation, bio pump, and Sr tracers
    ...
    def __init__(self):
        ...
        self.Nsurf = 2
        self.Ninterior  = 1
        self.EM = np.array([[-1,0,0], # high lat, low lat, deep
                            [0,-1,0],
                            [1,1,0]]) # Export matrix; fraction of export from surface (column) to interior (row)
    ...
    def TestTM(self):
        ...
        for year in range(1,1001):
            UnitTracer = np.matmul(self.TM4concentrations,UnitTracer)
